        ...
        return (
          <SortableCard key={card.id} id={card.id}>
            <QuickActionsCard
              id={card.id}
              title={card.title}
              actions={card.config?.actions || [
                { label: "Create Resource", icon: PlusIcon, onClick: () => console.log("Create") },
                { label: "View Logs", icon: ChartBarIcon, onClick: () => console.log("Logs") },
                { label: "Settings", icon: CogIcon, onClick: () => console.log("Settings") },
              ]}
              size={card.size}
              onRemove={removeCard}
            />
        ...
        return (
          <SortableCard key={card.id} id={card.id}>
            <RecentActivityCard
              id={card.id}
              title={card.title}
              activities={card.config?.activities || [
                {
                  id: "1",
                  action: "Deployed",
        ...
        <div className="flex-1 p-6 overflow-auto">
          ...
          {cards.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">Start building your dashboard by adding some cards</p>
              <AddCardDialog />
            </div>
          )}
        </div>
