...
export class StorageService {
    ...
    async createBucket(userId, data) {
        ...
        try {
            ...
            if (data.public) {
                ...
                await this.minioClient.setBucketPolicy(uniqueBucketName, JSON.stringify(policy));
            }
    ...
    async uploadFile(userId, bucketName, data, fileBuffer) {
        ...
        try {
            // Upload to MinIO
            const uploadResult = await this.minioClient.putObject(bucketName, data.key, fileBuffer, fileBuffer.length, {
    ...
    async getFile(userId, bucketName, key) {
        // Verify bucket ownership
        const bucket = await prisma.bucket.findFirst({
            where: { name: bucketName, userId },
        });
    ...
    async listObjects(userId, bucketName, prefix) {
        // Verify bucket ownership
        const bucket = await prisma.bucket.findFirst({
            where: { name: bucketName, userId },
        });
        ...
        try {
            const objects = [];
            const stream = this.minioClient.listObjects(bucketName, prefix || "", false);
            for await (const obj of stream) {
                objects.push({
                    key: obj.name,
                    size: obj.size,
    ...
    async deleteObject(userId, bucketName, key) {
        // Verify bucket ownership
        const bucket = await prisma.bucket.findFirst({
            where: { name: bucketName, userId },
        });
    ...
    async generatePresignedUrl(userId, bucketName, key, expiry = 3600) {
        // Verify bucket ownership
        const bucket = await prisma.bucket.findFirst({
            where: { name: bucketName, userId },
        });
        ...
        try {
            const url = await this.minioClient.presignedGetObject(bucketName, key, expiry);
    ...
    generatePublicReadPolicy(bucketName) {
        return {
            Version: "2012-10-17",
            Statement: [
                {
